# Hermes Curator — Exact Prompts (source-verified)

**Source:** `~/.hermes/hermes-agent/agent/curator.py`
**Extracted:** 2026-08-16, from the live installed Hermes Agent source tree.

The curator is Hermes' background skill-lifecycle system. It has two parts:

1. **Deterministic prune** — pure logic (no LLM), marks idle skills stale / archives them.
2. **LLM consolidation pass** ("umbrella-building") — opt-in (`curator.consolidate: true` or
   `hermes curator run --consolidate`), forks a background agent with the review prompt below.

Only the LLM pass is relevant to prefr Phase 2 (Reflection) — it's the closest Hermes
analogue to what Reflection does for preferences: it *creates/merges/consolidates*
durable artifacts (skills ↔ policies) from accumulated usage, with hard anti-bloat rules.

---

## `CURATOR_DRY_RUN_BANNER` (verbatim)
...
## `CURATOR_REVIEW_PROMPT` (verbatim)
...
The curator prompt encodes exactly the anti-bloat philosophy Sir wants for Reflection:

| Curator concept | prefr Reflection equivalent |
|---|---|
| "class-level umbrella beats N narrow siblings" | merge/update existing policies before creating new ones |
| "DO NOT delete — archive is max" | policies are never destroyed, only updated/merged |
| "judge overlap on CONTENT, not usage counters" | don't skip a merge just because a policy is rarely injected |
| "'each has a distinct trigger' is the wrong bar" | don't refuse to merge two policies just because their domains differ |
| "expected output: real umbrella-ification, <10 archives = stopped early" | direct mode must actually produce updates, not just re-report |
| structured `consolidations:` / `prunings:` YAML block | the direct-mode output contract (update/merge/create decisions) |

The one big difference: the curator operates on a **flat candidate list it scans every run**, whereas
Reflection operates on a **session window** (the last N turns). That's what the `direct` vs `deep`
split in Sir's idea maps to — direct = short window, frequent; deep = long window, rare.
